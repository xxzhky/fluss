 ...
 *
 * <p>Implementation note: we can add methods to our implementations of CRC32 and CRC32C, but we
 * cannot do the same for the Java implementations (we prefer the Java 9 implementation of CRC32C if
 * available). A utility class is the simplest way to add methods that are useful for all Checksum
 * implementations.
 *
 * <p>NOTE: This class is inspired from Kafka project.
 */
public final class Checksums {
    ...
    /**
     * Uses {@link Checksum#update} on {@code buffer}'s content, without modifying its position and
     * limit.<br>
     * This is semantically equivalent to {@link #update(Checksum, ByteBuffer, int, int)} with
     * {@code offset = 0}.
     */
    ...
    /**
     * Uses {@link Checksum#update} on {@code buffer}'s content, starting from the given {@code
     * offset} by the provided {@code length}, without modifying its position and limit.
     */
