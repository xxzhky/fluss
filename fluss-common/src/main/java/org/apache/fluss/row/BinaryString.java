...
import static org.apache.fluss.row.BinarySegmentUtils.allocateReuseBytes;
import static org.apache.fluss.row.BinarySegmentUtils.allocateReuseChars;
...
public final class BinaryString extends BinarySection
    ...
    BinaryString(MemorySegment[] segments, int offset, int sizeInBytes) {
    ...
    public String toString() {
        ...
        BinarySegmentUtils.copyToBytes(segments, offset, bytes, 0, sizeInBytes);
    ...
    public BinaryString copy() {
        byte[] copy = BinarySegmentUtils.copyToBytes(segments, offset, sizeInBytes);
    ...
    private BinaryString substringMultiSegs(final int start, final int until) {
        ...
        if (i > j) {
            return fromBytes(BinarySegmentUtils.copyToBytes(segments, offset + j, i - j));
        } else {
            return EMPTY_UTF8;
        }
    }
    ...
    BinaryString copyBinaryString(int start, int end) {
        int len = end - start + 1;
        byte[] newBytes = new byte[len];
        BinarySegmentUtils.copyToBytes(segments, offset + start, newBytes, 0, len);
        return fromBytes(newBytes);
    }
