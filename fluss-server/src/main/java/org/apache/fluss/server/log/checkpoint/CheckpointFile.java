...
/**
 * This class represents a utility to capture a checkpoint in a file. It writes down to the file in
 * the below format.
 *
 * <pre>
 *  ========= File beginning =========
 ...
 * </pre>
 *
 * <p>Each entry is represented as a string on each line in the checkpoint file. {@link
 * EntryFormatter} is used to convert the entry into a string and vice versa.
 *
 * @param <T> entry type.
 */
@Internal
public final class CheckpointFile<T> {
    ...
    public void write(Collection<T> entries) throws IOException {
        synchronized (lock) {
            ...
            FileUtils.atomicMoveWithFallback(tempPath, absolutePath);
        }
    }
